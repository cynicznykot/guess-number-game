...
# =================================================================================
# 3. GAME CONTROLLER CLASS (handles user interaction)
# =================================================================================


class GameController:
    """
    A class that handles user interaction and game flow.

    This class manages the console interface, user input
    and coordinates the game logic.
    """

    def __init__(self):
        """Initialize the game controller."""
        self.game: Optional[GuessNumberGame] = None
    ...
    def greet(self) -> None:
        ...
        time.sleep(1)
        print("🎯 Your goal is to guess the number generated by the program.")
    ...
    def setup_game(self) -> None:
        """
        Set up the game by getting range from the user.

        Prompts for start and end values, validates them and generates
        a random secret number within the range.
        """
        while True:
            start = get_number("🖥️ Please enter the START number: ")
            end = get_number("🖥️ Please enter the END number: ")
            ...
            try:
                self.game = GuessNumberGame(start, end)
                print(f"🤔 I've guessed a number from {start} to {end}!")
                break
